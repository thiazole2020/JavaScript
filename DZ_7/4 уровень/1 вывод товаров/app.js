...
function clickHandler(event) {
    let pordEl = document.querySelector('.products');
    pordEl.innerHTML = '';
...
function showCategory(category) {
    document.querySelector('.products').insertAdjacentHTML('afterbegin', getProductMarkup(category));
}
...
function getProductMarkup(product) {
    ...
    for (let i in products[product]) {
        htmlCode += `<div class="product">
                            ...
                            <a href='https://example.com/producs/${products[product][i].id}'>Подробнее</a> 
                    </div> `;
